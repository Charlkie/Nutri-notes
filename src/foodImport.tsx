...
function BarcodeImport({ onReview }: { onReview: (draft: FoodDraft) => void }) {
  const [code, setCode] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const video = useRef<HTMLVideoElement>(null);
  useEffect(() => { let stream: MediaStream | undefined; let reader: ZXingReader | undefined; let stopped = false; let timer = 0; const Detector = window.BarcodeDetector; if (Detector) { void navigator.mediaDevices?.getUserMedia({ video: { facingMode: { ideal: "environment" } }, audio: false }).then(value => { stream = value; if (video.current) { video.current.srcObject = value; void video.current.play(); } const detector = new Detector({ formats: ["ean_13", "ean_8", "upc_a", "upc_e"] }); const scan = async () => { if (stopped || !video.current) return; try { const found = await detector.detect(video.current); if (found[0]?.rawValue) { setCode(found[0].rawValue); stopped = true; return; } } catch { /* camera frames may not be ready */ } timer = window.setTimeout(scan, 350); }; timer = window.setTimeout(scan, 700); }).catch(() => setError("Camera access is unavailable. Enter the barcode below.")); } else { void loadZxing().then(() => { if (stopped || !video.current || !window.ZXing) return; reader = new window.ZXing.BrowserMultiFormatReader(); return reader.decodeFromVideoDevice(undefined, video.current, result => { const value = result?.getText(); if (value) { setCode(value); reader?.reset(); } }); }).catch(() => setError("Camera scanning is unavailable. Enter the barcode below.")); } return () => { stopped = true; clearTimeout(timer); reader?.reset(); stream?.getTracks().forEach(track => track.stop()); }; }, []);
