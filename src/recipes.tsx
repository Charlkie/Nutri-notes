...
import { createSnapshot, roundCalories, roundMacro } from "./domain/nutrition";
...
function RecipeBuilder({ recipe, foods, categories, onClose }: {
    ...
    return <section className="recipe-builder">
      ...
      <div className="recipe-fields"><label>Recipe name<input value={name} onChange={e => setName(e.target.value)} autoFocus placeholder="e.g. Spaghetti bolognese"/></label><div><label>Category<select value={categoryId} onChange={e => setCategoryId(e.target.value)}>{categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>Recipe yield<input type="number" min="0.1" step="0.1" inputMode="decimal" value={yieldServings} onChange={e => setYieldServings(e.target.value)}/><small>servings</small></label></div></div>
...
function LoggedIngredientRow({ingredient,categoryColour,editing,selected,onSelect,onToggle,onQuantity}:{ingredient:LoggedRecipeIngredient;categoryColour?:string;editing:boolean;selected:boolean;onSelect:()=>void;onToggle:(value:boolean)=>void;onQuantity:(value:number)=>void}) {
  ...
  return <div ref={setNodeRef} onClick={editing?onSelect:undefined} {...cardActivation} className={`ingredient-card ${editing?"editing":""} ${ingredient.enabled?"":"disabled"} ${selected?"selected":""} ${isDragging?"ingredient-dragging":""}`} style={{"--cat":categoryColour,transform:CSS.Transform.toString(transform),transition} as React.CSSProperties}>
    ...
    <span><strong>{ingredient.snapshot.name}</strong><small>{ingredient.group&&<b className="instance-group">{ingredient.group} · </b>}{roundCalories(ingredient.snapshot.calories)} kcal</small></span>
...
export function RecipeEntryEditor({ entry, onClose, onSaved, onDelete }: {
    ...
    return <main className="screen recipe-instance">
      ...
      {adding&&<div className="dialog-backdrop"><div className="dialog instance-add-dialog"><h2>Add ingredient this time</h2><p>This changes only this logged meal.</p><label className="dialog-search"><Search /><input type="search" value={query} onChange={e=>setQuery(e.target.value)} autoFocus placeholder="Search saved foods"/></label><div className="instance-food-results">{matches.map(food=><button key={food.id} onClick={()=>addFood(food)}><span><strong>{food.name}</strong><small>{food.baseQuantity} {food.baseUnit} · {roundCalories(food.calories)} kcal</small></span><Plus /></button>)}</div><button className="inline-create" onClick={()=>setCustomFood(true)}><Plus />Create new food</button><div><button onClick={()=>setAdding(false)}>Cancel</button></div></div></div>}
