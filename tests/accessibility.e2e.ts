...
test("the tracking page contains vertical scrolling above the bottom navigation",async({page})=>{
  ...
  const safeBottom = await page.evaluate(() => ({
    measured: Number(document.documentElement.dataset.safeBottom),
    applied: Number.parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--safe-bottom")),
  }));
...
test("Australian fast food catalogue finds Chicken Rappa offline and requires review", async ({ page }) => {
  ...
  await page.getByRole("button",{name:/Oporto.*152 menu items/i}).click();
  await page.getByPlaceholder("Search Oporto menu").fill("Chicken Rappa");
  const result = page.getByRole("button", { name: /Chicken Rappa.*Oporto.*421 kcal.*P —.*C —.*F —/i }).first();
  ...
  await expect(page.getByRole("spinbutton", { name: "Energy in kilocalories" })).toHaveValue("421");
...
test("large official Australian restaurant menus are searchable offline",async({page})=>{
  ...
  await page.getByRole("button",{name:/Import label/i}).click();
  await page.getByRole("button",{name:/Australian fast food/i}).click();
  await page.getByRole("button",{name:/Domino's.*259 menu items.*offline/i}).click();
  await page.getByRole("searchbox",{name:/Search Domino's menu/i}).fill("margherita classic crust");
  await page.getByRole("button",{name:/MARGHERITA — Classic Crust Domino's.*116 kcal.*P 5.*C 16.4.*F 3.1/i}).click();
  await expect(page.getByLabel("Food name")).toHaveValue("MARGHERITA — Classic Crust");
  await page.getByRole("button",{name:"Back"}).click();
  await page.getByRole("button",{name:/Australian fast food/i}).click();
  await page.getByRole("button",{name:/Pizza Hut.*121 menu items.*offline/i}).click();
  await page.getByRole("searchbox",{name:/Search Pizza Hut menu/i}).fill("margherita large original pan");
  await page.getByRole("button",{name:/Margherita — Large Original Pan Pizza Hut.*1624 kcal.*P —.*C —.*F —/i}).click();
  await expect(page.getByLabel("Food name")).toHaveValue("Margherita — Large Original Pan");
  await expect(page.getByLabel("Protein (g)")).toHaveAttribute("placeholder","Not published");
  await page.getByRole("button",{name:"Back"}).click();
  await page.getByRole("button",{name:/Australian fast food/i}).click();
  await page.getByRole("button",{name:/Nando's.*21 menu items.*offline/i}).click();
  await page.getByRole("searchbox",{name:/Search Nando's menu/i}).fill("half peri peri chicken");
  await page.getByRole("button",{name:/Half PERi-PERi Chicken Nando's.*715 kcal.*P 108.*C 1.1.*F 30.8/i}).click();
  await expect(page.getByLabel("Food name")).toHaveValue("Half PERi-PERi Chicken");
  await expect(page.getByLabel("Protein (g)")).toHaveValue("108");
  await page.getByRole("button",{name:"Back"}).click();
  await page.getByRole("button",{name:/Australian fast food/i}).click();
  ...
  await page.getByRole("button",{name:"Back"}).click();
  await page.getByRole("button",{name:/Australian fast food/i}).click();
  await page.getByRole("button",{name:/Red Rooster.*188 menu items.*offline/i}).click();
  await page.getByRole("searchbox",{name:/Search Red Rooster menu/i}).fill("reds burger");
  await page.getByRole("button",{name:/^Reds Burger Red Rooster.*798 kcal.*P —.*C —.*F —/i}).click();
  await expect(page.getByLabel("Food name")).toHaveValue("Reds Burger");
  await expect(page.getByLabel("Protein (g)")).toHaveAttribute("placeholder","Not published");
  await page.getByRole("button",{name:"Back"}).click();
  await page.getByRole("button",{name:/Australian fast food/i}).click();
...
test("weight CSV import preserves same-day readings and overlays nutrition trends", async ({ page }) => {
  ...
  const weightPoint = page.getByRole("button", { name: /Weight on 31 July 2026: average 68\.8 kg, minimum 68\.4, maximum 69\.2, 2 readings/ });
  await weightPoint.focus();
  await weightPoint.press("Enter");
  await expect(chartTooltip).toContainText("Weight 68.8 kg average");
  await expect(chartTooltip).toContainText("68.4–69.2 kg · 2 readings");
  const scrubArea = page.locator(".chart-gesture-surface");
  await expect(scrubArea).toHaveCSS("touch-action", "none");
  await scrubArea.scrollIntoViewIfNeeded();
  const scrubBounds = await scrubArea.boundingBox();
  expect(scrubBounds).not.toBeNull();
  const chartScroll = page.locator(".auxiliary-overlay");
  const chartScrollBefore = await chartScroll.evaluate((element) => element.scrollTop);
  await swipeScreen(
    page,
    { x: scrubBounds!.x + scrubBounds!.width / 2, y: scrubBounds!.y + scrubBounds!.height * 0.6 },
    { x: scrubBounds!.x + scrubBounds!.width / 2, y: scrubBounds!.y + scrubBounds!.height * 0.4 },
  );
  expect(await chartScroll.evaluate((element) => element.scrollTop)).toBe(chartScrollBefore);
  await swipeScreen(
    page,
    { x: scrubBounds!.x + scrubBounds!.width - 2, y: scrubBounds!.y + scrubBounds!.height / 2 },
    { x: scrubBounds!.x + 2, y: scrubBounds!.y + scrubBounds!.height / 2 },
  );
  ...
  expect((await page.locator(".analytics-line-large svg").boundingBox())?.height).toBeGreaterThanOrEqual(420);
